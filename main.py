...
## Weapon
st.subheader("Weapon")
weapon_type = st.selectbox("Choose Weapon Type",["Ancient Empress","Necro","Fafnir"])
if weapon_type != "Fafnir":
    hat_stat = st.radio("Choose Stat",["Boss Atk","Crit Dmg","Crit Rate","Exp"])
else:
    hat_stat = "None"
weapon_level = st.radio("Weapon Level",[30,40,50])

## Hat
st.subheader("Hat Selection")
hat_type = st.selectbox("Choose Hat Type",["Ancient Empress","Necro","Fafnir"])
if hat_type != "Fafnir":
    hat_stat = st.radio("Choose Stat",["Boss Atk","Crit Dmg","Crit Atk","Exp"])
else:
    hat_stat = "None"
hat_level = st.radio("Hat Level",[30,40,50])

## Secondary Weapon
st.subheader("Secondary Weapon Selection")
swep_type = st.selectbox("Choose Secondary Weapon Type",["Unique Longinus Spear","Unique Normal","Legendary Longinus Spear","Legendary Normal","Mythic Longinus Spear","Mythic Normal"])
