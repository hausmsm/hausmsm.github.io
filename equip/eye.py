...
class eye:
    def __init__(self):
        ...
        with st.beta_expander("Eye Accessory"):
            eye_list = st.selectbox("Choose an Eye Accessory", ["Black Bean Mark","Chaos Pink Bean Mark (Unique)","Chaos Pink Bean Mark (Legendary)"])
            self.type = eye_list
            _, eye1, _ = st.beta_columns([0.02, 0.96, 0.02])
            if "Black Bean Mark" in [eye_list]:
                # Base
                self.hp += 3000
                self.cd += 3
                self.bosssetcount += 1
            elif "Chaos Pink Bean Mark (Unique)" in [eye_list]:
                # Base
                self.hp += 4500
                self.cd += 5
                self.bosssetcount += 1
            elif "Chaos Pink Bean Mark (Legendary)" in [eye_list]:
                # Base
                self.hp += 6000
                self.cd += 6
                self.bosssetcount += 1
